...
{   // Начало тела объявления пространства имен
    ...
    public delegate void Display();  // описание делегата
    ...
    {   // начало тела класса 
        ...
        {   // начало тела метода Main
            ...
            Stat stat = new Stat();
            Print menuDel = delegate (string str)
            {
            ...
            menuDel(menu);  // вывод в консоль текстового меню через анонимный метод
            ...
            {   // начало тела цикла
                ...
                {   // начало тела логического условия
                    Display printMemory = () => // объявление анонимного метода через лямбда-выражение
                    {   // начало анонимного метода
                        Console.WriteLine("[delegated call]");  //  указание на делегацию метода
                    ...
                    };  // конец анонимного метода
                    printMemory();  // вызов анонимного метода через делегат
                ...
                {   // начало тела логического условия
                    ...
                    Display countPrint = delegate () // объявление анонимного метода
                    {   // начало анонимного метода
                        Console.WriteLine("[delegated call]");  //  указание на делегацию метода
                        stat.Display();     // вывод состояния счетчиков в консоль
                    };  // конец анонимного метода
                    countPrint();   // вызов анонимного метода через делегат
                }   // конец тела логического условия
                ...
                {   // начало тела логического условия
                    ...
                    "3 - Call for bird\n" +
                    "Any key to cancel"
                    );      // вывод вспомогательного меню
                    key = Console.ReadLine();   // чтение символа с клавиатуры
